...
- Survey engine (Faza 5): `SURVEY_ENGINE_URL=http://survey-engine:8000` (frontend → Python FastAPI)
- Survey AI keys (serviciu `survey-engine`): `DEEPSEEK_API_KEY`, `ANTHROPIC_API_KEY`

`docker/coolify.yml` pornește `survey-engine` cu volume `survey_output` pentru PDF-uri. Dacă rulezi doar
`Dockerfile` root (fără compose), adaugă manual o aplicație Coolify din `survey-engine/Dockerfile`.
...
**Sursă Git producție:** `https://gitea.com/Solaris-Cet/solaris-clean` (nu GitHub).

În Coolify:

- Source: Gitea → `Solaris-Cet/solaris-clean`, branch `main`
