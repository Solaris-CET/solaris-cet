...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (auth instanceof Response) {
    await recordPublicApiUsage({ apiKeyId: null, userId: null, method: req.method, path: '/api/v1/transactions', status: auth.status, latencyMs: Date.now() - start });
    return auth;
  }

  const d = decideRateLimit({ req, bucket: 'public-v1-transactions', keyPart: auth.apiKeyId, limit: req.method === 'POST' ? 30 : 120, windowSeconds: 60 });
  if (!d.ok) {
    await recordPublicApiUsage({ apiKeyId: auth.apiKeyId, userId: auth.userId, method: req.method, path: '/api/v1/transactions', status: 429, latencyMs: Date.now() - start });
  ...
  if (req.method === 'GET') {
    ...
    const body = { version: 'v1', items, nextCursor };
    const status = 200;
    await recordPublicApiUsage({ apiKeyId: auth.apiKeyId, userId: auth.userId, method: req.method, path: '/api/v1/transactions', status, latencyMs: Date.now() - start });
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      await recordPublicApiUsage({ apiKeyId: auth.apiKeyId, userId: auth.userId, method: req.method, path: '/api/v1/transactions', status: 400, latencyMs: Date.now() - start });
    ...
    const status = 201;
    await recordPublicApiUsage({ apiKeyId: auth.apiKeyId, userId: auth.userId, method: req.method, path: '/api/v1/transactions', status, latencyMs: Date.now() - start });
    return jsonResponsePublic(req, { version: 'v1', transaction: tx }, status, rateLimitHeaders(d));
  }

  await recordPublicApiUsage({ apiKeyId: auth.apiKeyId, userId: auth.userId, method: req.method, path: '/api/v1/transactions', status: 405, latencyMs: Date.now() - start });
