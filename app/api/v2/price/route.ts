...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (auth instanceof Response) {
    await recordPublicApiUsage({ apiKeyId: null, userId: null, method: req.method, path: '/api/v2/price', status: auth.status, latencyMs: Date.now() - start });
    return auth;
  }

  const d = decideRateLimit({ req, bucket: 'public-v2-price', keyPart: auth.apiKeyId, limit: 180, windowSeconds: 60 });
  if (!d.ok) {
    await recordPublicApiUsage({ apiKeyId: auth.apiKeyId, userId: auth.userId, method: req.method, path: '/api/v2/price', status: 429, latencyMs: Date.now() - start });
  ...
  const body = {
    version: 'v2',
    asset: { symbol: 'CET', chain: 'TON' },
    price: { usd: priceUsd, decimals: 6 },
    updatedAt: new Date().toISOString(),
    source: priceUsd ? 'env' : 'fallback',
  };
  const status = 200;
  await recordPublicApiUsage({ apiKeyId: auth.apiKeyId, userId: auth.userId, method: req.method, path: '/api/v2/price', status, latencyMs: Date.now() - start });
