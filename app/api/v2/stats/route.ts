...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (auth instanceof Response) {
    await recordPublicApiUsage({ apiKeyId: null, userId: null, method: req.method, path: '/api/v2/stats', status: auth.status, latencyMs: Date.now() - start });
    return auth;
  }

  const d = decideRateLimit({ req, bucket: 'public-v2-stats', keyPart: auth.apiKeyId, limit: 60, windowSeconds: 60 });
  if (!d.ok) {
    await recordPublicApiUsage({ apiKeyId: auth.apiKeyId, userId: auth.userId, method: req.method, path: '/api/v2/stats', status: 429, latencyMs: Date.now() - start });
    return rateLimitedResponsePublic(req, d);
  }

  const sinceMs = Date.now() - 24 * 60 * 60 * 1000;
  const global = aggregatePublicApiUsage({ sinceMs, pathPrefix: '/api/v2/' });
  const mine = aggregatePublicApiUsage({ sinceMs, apiKeyId: auth.apiKeyId, pathPrefix: '/api/v2/' });
  const body = {
    version: 'v2',
    window: { since: new Date(sinceMs).toISOString(), until: new Date().toISOString() },
    usage: {
      global,
      byApiKey: [{ apiKeyId: auth.apiKeyId, ...mine }],
    },
  };
  const status = 200;
  await recordPublicApiUsage({ apiKeyId: auth.apiKeyId, userId: auth.userId, method: req.method, path: '/api/v2/stats', status, latencyMs: Date.now() - start });
