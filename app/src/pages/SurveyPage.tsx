...
import { clearSurveyDraft, storedToPhotos } from '@/lib/surveyDraftStorage';
import {
  canGenerateSurvey,
  checklistStatusLabel,
  MAX_SURVEY_PHOTOS,
  scoreRingColor,
  SURVEY_PAGE_DESCRIPTION,
...
const SURVEY_TAB_IDS = SURVEY_TABS;
...
export default function SurveyPage() {
  const [tab, setTab] = useState<SurveyTab>('report');
  ...
  useEffect(() => {
    ...
    setCalcPrefillNote(SURVEY_PAGE_MESSAGES.calcPrefillNote);
  }, []);
  ...
  const addPhotos = useCallback((files: FileList | File[]) => {
    ...
    setPhotos((prev) => {
      const next = [...prev, ...incoming].slice(0, MAX_SURVEY_PHOTOS);
      return next;
    });
    setPreviews((prev) => {
      const urls = incoming.map((f) => URL.createObjectURL(f));
      return [...prev, ...urls].slice(0, MAX_SURVEY_PHOTOS);
    });
    setError('');
  }, []);
  ...
  const handleGpsCapture = useCallback(() => {
    if (!navigator.geolocation) {
      setError(SURVEY_PAGE_MESSAGES.gpsUnavailable);
      return;
    }
    setGpsLoading(true);
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setGpsLoading(false);
        setError(SURVEY_PAGE_MESSAGES.gpsFailed);
      },
  ...
  const canGenerate = useMemo(
    () => canGenerateSurvey(form.clientName, form.clientCity, photos.length),
  ...
  const handleDemo = async () => {
    ...
    try {
      ...
      setError(toSurveyPageError(err, 'Demo eșuat'));
    } finally {
      setGenerating(false);
    }
  };
  ...
  const handleGenerate = async () => {
    if (!canGenerate) {
      setError(SURVEY_PAGE_MESSAGES.generateIncomplete);
      return;
    }
    if (!online) {
      try {
        ...
        setError('');
        setProgress(SURVEY_PAGE_MESSAGES.offlineQueued);
      } catch {
        setError(SURVEY_PAGE_MESSAGES.offlineSaveFailed);
      }
      return;
    }
    setGenerating(true);
    ...
    try {
      ...
      setError(toSurveyPageError(err, 'Eroare la generare'));
    } finally {
      setGenerating(false);
    }
  };
  ...
  const handleCrm = async () => {
    ...
    try {
      ...
      setError(toSurveyPageError(err, 'CRM eșuat'));
    }
  };
  ...
  const handleCorrection = async () => {
    ...
    try {
      ...
      setError(toSurveyPageError(err, 'Corecție eșuată'));
    } finally {
      ...
      <main className="relative mx-auto max-w-5xl px-4 pb-16 pt-24 sm:px-6">
        <header className="mb-8 text-center">
          ...
          {engineOk !== null && online && (
            <p className={cn('mt-2 text-center text-xs', engineOk ? 'text-emerald-400' : 'text-amber-400')}>
              {engineOk ? SURVEY_PAGE_MESSAGES.engineConnected : SURVEY_PAGE_MESSAGES.engineDemo}
            </p>
          )}
        </header>
        ...
        {costBudgetAlert ? (
          <p className="mb-4 rounded-xl border border-amber-400/25 bg-amber-400/10 px-4 py-2 text-sm text-amber-100">
            {SURVEY_PAGE_MESSAGES.costBudgetAlert}
          </p>
        ) : null}
        {calcPrefillNote ? (
        ...
        <div
          className="mb-6 flex gap-2 rounded-2xl border border-white/10 bg-white/[.03] p-1"
          role="tablist"
          aria-label="Secțiuni survey"
        >
          {SURVEY_TAB_IDS.map((t) => (
            <button
              key={t}
              type="button"
              role="tab"
              aria-selected={tab === t}
              aria-controls={`survey-tab-${t}`}
              onClick={() => setTab(t)}
              className={cn(
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setError(toSurveyPageError(err, 'Batch eșuat'));
                } finally {
            ...
            <section className="space-y-5">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[.03] p-5">
                <div className="mb-4 flex items-center gap-2 text-sm font-semibold text-amber-200">
                  ...
                  Poze șantier ({photos.length}/{MAX_SURVEY_PHOTOS})
                </div>
                <div
                  role="button"
                  tabIndex={0}
                  aria-label="Încarcă poze șantier — apasă sau trage fișiere aici"
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {([
                    ...
                    <label key={key} className="block text-xs text-white/50">
                      ...
                      >
                        {STATUS.map((s) => <option key={s} value={s}>{checklistStatusLabel(s)}</option>)}
                      </select>
                    </label>
                  ))}
                </div>
            ...
            <aside className="space-y-5 lg:sticky lg:top-24 lg:self-start">
              <div className="rounded-2xl border border-white/10 bg-white/[.03] p-5">
                <button
                  ...
                  aria-label={online ? 'Generează raport PDF' : 'Salvează raport în coada offline'}
