  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!res.ok || !data.ok) {
          setState({ kind: 'error', message: data.error || 'Token invalid sau expirat.' });
          return;
        }
        setState({ kind: 'ok', message: 'Abonarea a fost confirmată. Welcome + onboarding vor urma.' });
      } catch {
  ...
  return (
    <main id="main-content" tabIndex={-1} className="min-h-[70vh] flex items-center justify-center px-6 py-20">
      <div className="max-w-xl w-full rounded-2xl border border-white/10 bg-black/30 p-6">
        <h1 className="text-white text-2xl font-semibold tracking-tight">Confirmare newsletter</h1>
        <div className="mt-4 flex items-start gap-3">
          ...
          <p className="text-sm text-white/70 leading-relaxed">
            {state.kind === 'loading' ? 'Se confirmă…' : state.message}
          </p>
        </div>
